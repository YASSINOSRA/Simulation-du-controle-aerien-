...
<body>
 <li><a href="home.jsp">Home</a></li>
    ...
    function createCanvas(width, height, set2dTransform = true) {
      ...
      canvas.setAttribute("style",`background-image: url(worl.svg);
  ...
  let airoprts=[
      new Airport("Rabat",new Point (600, 400)),
      new Airport("Greeland",new Point (540, 106)),
      new Airport("Brazil",new Point (433, 523)),
      new Airport("newYork",new Point(331, 315)),
      new Airport("Italy",new Point (701, 357)),
      new Airport("Australia",new Point (1000, 610)),
      new Airport("Russia",new Point (972, 260)),
      new Airport("Canada",new Point (288, 193)),
      new Airport("Zambia",new Point (706, 519))
              ];
  ...
  function blinkDot(plain) {
	  ...
	  var radius = 3;
	    ...
	    cnx.beginPath();
	    cnx.arc(plain.point.x, plain.point.y, radius, 0, 6 * Math.PI);
	    if (color) {
	      cnx.fillStyle = "red";
	    } else {
	      cnx.fillStyle = "blue";
...
let plains=[
  ...
  function movePlain(){
    ...
    setInterval(()=>{
     ...
     fetch("http://localhost:8080/Air-Traffic/update")
  ...
  function isMouseOverRectangleAir(mouseX, mouseY,air) {
	...
	function slowFog(){
		...
		if (plain.point.x > fog.x - 50 && plain.point.x < fog.x + 50 && plain.point.y > fog.y - 50 && plain.point.y < fog.y + 50) {
	          ...
	          plain.fog=true;
	          let s=.5;
	          fetch("http://localhost:8080/Air-Traffic/update?speed="+s+"&name="+plain.name)
	...
	function drawAiroportInfo(airport){
	    ...
	    displayText2(airx+5,airy+39, "Places au sol: " + airport.places_au_sol);
  ...
  function createFog(point) {
      ...
      fogGradient.addColorStop(0, "rgba(255, 0, 0, 1)");
      fogGradient.addColorStop(1, "rgba(255, 0, 0, 0)");
